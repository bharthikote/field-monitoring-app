...
import { searchAgroDealers, listAgroDealers } from '../api';
...
// Reached only from Home's Agro Dealer Visit card - always has a back link,
// same shape as InstitutionsListScreen.
export default function AgroDealersListScreen({ token, onBack, onCreateNew, onSelectDealer }) {
  ...
  const loadAll = async () => {
    ...
    setError('');
    try {
      const data = await listAgroDealers(token);
      setResults(data.agroDealers);
      setSearchedQuery('');
    } catch (err) {
  ...
  const handleSearch = async (searchQuery) => {
    ...
    setError('');
    try {
      const data = await searchAgroDealers(token, trimmed);
      setResults(data.agroDealers);
      ...
      <View style={styles.header}>
        ...
        {!searchOpen ? (
          <View style={styles.titleRow}>
            <Text style={styles.title}>Select Agro Dealer</Text>
        ...
        ListHeaderComponent={
          results !== null && results.length > 0 ? (
            <Text style={styles.sectionLabel}>
              {searchedQuery ? `Results for "${searchedQuery}"` : `All Agro Dealers (${results.length})`}
            </Text>
          ) : null
        }
        ListEmptyComponent={
          !loading && results !== null ? (
            <Text style={styles.empty}>
              {searchedQuery ? 'No agro dealers matched your search.' : 'No agro dealers registered yet.'}
            </Text>
          ) : null
        }
